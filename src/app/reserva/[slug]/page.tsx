...
export default function ReservaPublicaDinamicaPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setLoading(true);
                // Fetch Studio Info
                const studioRes = await fetch(`${API_URL}/public/tenants/${slug}`);
                if (!studioRes.ok) throw new Error('Estúdio não encontrado');
                const studioData = await studioRes.json();
                setStudioInfo(studioData);

                // Auto-select artist if solo
                if (studioData.artists?.length === 1) {
                    setFormData(prev => ({ ...prev, artistId: studioData.artists[0].id }));
                }

                // Fetch Portfolio
                const portfolioRes = await fetch(`${API_URL}/portfolio/public/${slug}`);
                if (portfolioRes.ok) {
    ...
    const handleNext = async () => {
        if (step < 3) setStep(step + 1);
            ...
            try {
                const selectedArtist = studioInfo.artists.find((a: any) => a.id === formData.artistId);
                const description = `WhatsApp: ${formData.whatsapp}\n\nIdeia: ${formData.idea}\n\nDisponibilidade: ${formData.availability}${selectedArtist ? `\n\nArtista Selecionado: ${selectedArtist.name}` : ''}`;

                const res = await fetch(`${API_URL}/public/budgets/${slug}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        title: `Solicitação: ${formData.idea.substring(0, 30)}...`,
                        clientName: formData.name,
                        description,
                        images: formData.images,
                        clientEmail: '' // We could add this field to step 1
                    })
                });
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-[#0A0A0A] flex flex-col items-center justify-center space-y-4">
                ...
                <p className="text-zinc-500 font-black uppercase tracking-[0.3em] text-[10px]">Carregando Estúdio...</p>
            </div>
        );
    }
    ...
    if (error || !studioInfo) {
        return (
            <div className="min-h-screen bg-[#0A0A0A] flex flex-col items-center justify-center p-6 text-center">
                <X className="w-16 h-16 text-rose-500 mb-6 opacity-20" />
                <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter">Ops! Algo deu errado</h2>
                <p className="text-zinc-500 mt-2 max-w-sm">{error || 'Não conseguimos localizar este estúdio.'}</p>
                <button onClick={() => window.location.reload()} className="mt-8 px-8 py-4 bg-zinc-900 border border-premium-border text-white rounded-xl font-bold uppercase text-xs tracking-widest">Tentar Novamente</button>
            </div>
        );
    }
            ...
            <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center p-6 bg-[url('/stipple.png')]">
                <div className="w-full max-w-lg premium-card text-center space-y-6 animate-premium-fade py-12">
                    ...
                    <p className="text-secondary-text font-medium leading-relaxed">
                        Recebemos seu pedido de orçamento para o estúdio {studioInfo.name}. Nossa equipe analisará as referências e entrará em contato via WhatsApp em breve.
                    </p>
                    <div className="pt-4">
                        <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em] mb-4 text-center">Protocolo: #INK-{Math.floor(Math.random() * 90000 + 10000)}</p>
                        <button onClick={() => window.location.reload()} className="w-full bg-zinc-900 border border-premium-border text-white px-8 py-4 rounded-xl font-bold hover:bg-zinc-800 transition-all uppercase text-xs tracking-widest">
                            Nova Solicitação
                        </button>
                    </div>
                </div>
    ...
    return (
        <div className="min-h-screen bg-[#0A0A0A] flex flex-col p-6 overflow-hidden relative">
                ...
                <div className="w-full max-w-2xl">
                    ...
                    <div className="premium-card bg-[#111111]/80 backdrop-blur-xl border-premium-border/50 shadow-2xl p-8 md:p-12 relative overflow-hidden">
                        ...
                        {step === 1 && (
                            <div className="space-y-8 animate-premium-fade">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    {/* ARTIST SELECTION - Only show if more than 1 artist */}
                                    {studioInfo.artists && studioInfo.artists.length > 1 && (
                                        <div className="md:col-span-2 space-y-2">
                                            <label className="text-[10px] font-black text-secondary-text uppercase tracking-widest ml-1">Escolha o Profissional</label>
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                                {studioInfo.artists.map((artist: any) => (
                                                    <button
                                                        key={artist.id}
                                                        onClick={() => setFormData({ ...formData, artistId: artist.id })}
                                                        className={`p-3 border rounded-xl text-[10px] font-bold transition-all uppercase tracking-widest text-center ${formData.artistId === artist.id ? 'bg-gold-polished border-gold-polished text-black shadow-lg translate-y-[-2px]' : 'bg-[#0A0A0A] border-premium-border text-white hover:border-gold-polished'}`}
                                                    >
                                                        {artist.name}
                                                    </button>
                                                ))}
                        ...
                        <div className="mt-12 flex gap-4">
                            ...
                            <button 
                                onClick={handleNext} 
                                disabled={step === 1 && !formData.name || !formData.whatsapp || (studioInfo.artists?.length > 1 && !formData.artistId)}
                                className="flex-1 btn-premium flex items-center justify-center gap-3 group uppercase text-xs tracking-[0.2em] disabled:opacity-50 disabled:cursor-not-allowed"
                            >
            ...
            <footer className="max-w-4xl w-full mx-auto py-8 border-t border-premium-border/30 flex flex-col md:flex-row justify-between items-center gap-4 relative z-10 opacity-60">
                <p className="text-[9px] font-medium text-zinc-500 uppercase tracking-widest">© 2026 {studioInfo.name.toUpperCase()} . Todos os direitos reservados.</p>
                    ...
                    <div className="w-full max-w-6xl h-full flex flex-col relative z-20 animate-premium-pop">
                        <div className="flex justify-between items-center mb-8">
                            <div>
                                ...
                                <p className="text-[10px] font-black text-gold-polished uppercase tracking-[0.3em]">Trabalhos Recentes em {studioInfo.name}</p>
                            </div>
                            <button
