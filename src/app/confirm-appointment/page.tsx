...
function ConfirmationContent() {
    ...
    const [message, setMessage] = useState('Confirmando seu agendamento...');
    ...
    useEffect(() => {
        ...
        const confirm = async () => {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/confirmations/appointment?token=${token}`);
                if (response.ok) {
                    setStatus('success');
                    setMessage('Seu agendamento foi confirmado com sucesso! Obrigado.');
                } else {
                    setStatus('error');
                    setMessage('Não foi possível confirmar seu agendamento. O link pode ter expirado ou ser inválido.');
                }
            } catch (error) {
                setStatus('error');
                setMessage('Ocorreu um erro ao tentar confirmar seu agendamento.');
            }
        };
